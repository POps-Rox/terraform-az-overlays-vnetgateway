...
resource "azurerm_route_table" "vgw" {
  ...
  name                = try(trimspace(var.route_table_name), "") != "" ? var.route_table_name : "${local.virtual_network_gateway_name}-rt"
  ...
  tags                          = merge(local.default_tags, var.route_table_tags)
}
...
resource "azurerm_public_ip" "vgw" {
  ...
  location            = var.location
  name                = try(trimspace(each.value.public_ip.name), "") != "" ? each.value.public_ip.name : "${local.virtual_network_gateway_name}-${each.key}-pip"
  resource_group_name = try(local.resource_group_name, var.existing_virtual_network_resource_group_name)
  sku                 = each.value.public_ip.sku
  tags                = merge(local.default_tags, each.value.public_ip.tags)
}
